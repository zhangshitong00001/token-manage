...
export default function UsageLog() {
  ...
  const apiEndpoint = isAdmin ? '/admin/usage/list' : '/user/my-usage-list'

  const loadData = (p = page) => {
    setLoading(true)
    const params = { page: p, page_size: 20 }
    if (isAdmin && userId) params.user_id = parseInt(userId)
    if (dates?.[0]) params.start_date = dates[0].format('YYYY-MM-DD')
    if (dates?.[1]) params.end_date = dates[1].format('YYYY-MM-DD')
    api.get(apiEndpoint, { params }).then(res => {
      setData(res.items || [])
  ...
  const columns = [
    ...(isAdmin ? [{ title: 'ID', dataIndex: 'id', width: 60 }] : []),
    ...
    { title: '请求ID', dataIndex: 'request_id', render: (v) => v ? <code style={{ fontSize: 11 }}>{v.slice(0, 20)}...</code> : '-' },
    { title: '时间', dataIndex: 'usage_time', render: (v) => v ? dayjs(v).format('YYYY-MM-DD HH:mm:ss') : '-' },
  ]
  ...
  return (
    <div>
      ...
      <Table
        ...
        rowKey="id"
        loading={loading}
        locale={{ emptyText: isAdmin ? '暂无消耗记录' : '暂无您的消耗记录' }}
