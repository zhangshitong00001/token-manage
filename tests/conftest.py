...
# ── Test fixture files ──────────────────────────────────────────────────────
# Static test files living under tests/fixtures/. Created once on disk;
# fixtures return their paths so tests can reference them without
...
@pytest.fixture()
def fixture_normal_file() -> Path:
    """Return the path to a normal text file with known content (10 lines).

    Content includes ASCII text, numbers, special characters, and Unicode.
    Useful for read/parse/hash/checksum tests.
    """
    return _FIXTURES_DIR / "normal_file.txt"


@pytest.fixture()
def fixture_empty_file() -> Path:
    """Return the path to an empty file (0 bytes).

    Useful for edge-case tests: read on empty file, upload, size checks.
    """
    return _FIXTURES_DIR / "empty_file.txt"


@pytest.fixture(scope="session")
def fixture_large_file() -> Path:
    """Return the path to a ~105 MiB file filled with zeros.

    Useful for large-file handling tests: streaming, chunked uploads,
    progress reporting, memory-mapped reads, and timeout behaviour.
    Session-scoped so the file is only stat'd/opened once per test run.
    """
    return _FIXTURES_DIR / "large_file.bin"
